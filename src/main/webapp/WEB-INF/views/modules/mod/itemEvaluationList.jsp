				...
				<td>
					<shiro:hasPermission name="mod:itemEvaluation:edit">
						...
						<c:if test="${row.type ne 'dimension'}">
							...
							<a href="${ctx}/mod/itemEvaluationDimension/list2?evaluation.id=${row.id}&category.id=${row.category.id}&evaluationId=${row.id}&categoryId=${row.category.id}">添加客观评价节点</a>
						</c:if>
					</shiro:hasPermission>							
				</td>			
			</tr>
