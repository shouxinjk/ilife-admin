...
<head>
	...
	<script type="text/javascript">
		...
		function registerEventListener(){
            ...
            $("input[data-section^='sec-']").each((index, item) => {
                ...
                $("#"+inputId).blur(function(e){
                    ...
                    $.ajax({
                        type: "POST",
                        url: "${ctx}/mod/userDimension/rest/weight",
                        data: data,        
        ...
        function updateSort() {
            loading('正在提交，请稍等...');
            $("#listForm").attr("action", "${ctx}/mod/userDimension/updateSort");
            $("#listForm").submit();
        }
	</script>
</head>
	...
	<table id="treeTableDimension" class="table table-striped table-bordered table-condensed">
		...
		<c:forEach items="${list}" var="row">
			<tr id="${row.id}" pId="${row.parent.id ne '1'?row.parent.id:'0'}">
				...
				<a href="${ctx}/mod/userDimensionMeasure/list?dimensionId=${row.id}&dimension.id=${row.id}">
					${row.name}
				</a></td>
				<td>
					${row.propKey}
				...
				<td>
					<shiro:hasPermission name="mod:userDimension:edit">
						<a href="${ctx}/mod/${row.type ne 'dimension'?'userDimensionMeasure':'userDimension'}/form?id=${row.id}">修改</a>
